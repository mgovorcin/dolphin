...
time scale with the *full* frame, not the block. With N blocks × 20 GSLCs,
...
def _open_for_bounds(first_file: str, subdataset: str | None):
    ...
    gdal.UseExceptions()
    path = (
        f'NETCDF:"{first_file}":{subdataset}' if subdataset else str(first_file)
    )
    ds = gdal.Open(path)
    if ds is None:
...
def _min_halo_rows(cfg: DisplacementWorkflow) -> int:
    ...
    sim_radius = cfg.phase_linking.similarity_search_radius
    ...
    *,
    ...
    try:
        ...
        if authority is None:
            raise RuntimeError(
                f"Could not read EPSG code from {first_file} projection"
            )
        epsg = int(authority)
    finally:
    ...
    if num_blocks == 1:
        ...
        return {
            "block_00": (
                (float(xmin), float(ymax - ny * px_y), float(xmax), float(ymax)),
                epsg,
            )
        }
    ...
    blocks: dict[str, tuple[tuple[float, float, float, float], int]] = {}
    for i in range(num_blocks):
        ...
        blocks[block_id] = (
            (float(xmin), float(block_ymin), float(xmax), float(block_ymax)),
            epsg,
        )
        logger.debug(
            "%s: rows central=[%d, %d] read=[%d, %d] y=[%.0f, %.0f]",
            block_id, cs, ce, rs, re, block_ymin, block_ymax,
        )

    logger.info(
        "Split frame (%dx%d px, EPSG:%d) into %d azimuth blocks, halo=%d rows",
        nx, ny, epsg, num_blocks, halo,
    )
    return blocks
