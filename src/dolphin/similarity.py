...
def create_similarity_per_date(
    ...
    for i, (slc_file, date_str) in enumerate(zip(slc_file_list, date_strs)):
        ...
        neighbor_idxs = [
            j for j in range(max(0, i - nearest_n), min(n_slcs, i + nearest_n + 1))
            if j != i
        ]
...
def create_per_date_similarities(
    ...
    ifg_file_list = [Path(f) for f in ifg_file_list]

    # Parse (ref_date, sec_date) pairs from filenames
    date_pattern = re.compile(r"(\d{8})")
    pairs: list[tuple[str, str]] = []
    for f in ifg_file_list:
